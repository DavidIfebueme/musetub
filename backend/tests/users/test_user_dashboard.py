...
@pytest.mark.asyncio
async def test_user_history_and_spending(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            content_title = upload.json()["title"]
            ...
            items = history.json()
            assert len(items) == 1
            assert items[0]["channel_id"] == channel_id
            assert items[0]["content_id"] == content_id
            assert items[0]["content_title"] == content_title
            assert items[0]["status"] == "closed"
            assert items[0]["total_seconds_streamed"] == 30
            assert items[0]["total_amount_owed"] == gross
            assert items[0]["total_amount_settled"] == gross

            spending = await client.get(
                "/api/v1/users/me/spending",
                headers={"Authorization": f"Bearer {user_token}"},
            )
            assert spending.status_code == 200
            payload = spending.json()
            assert payload["total_seconds_streamed"] == 30
            assert payload["total_amount_owed"] == gross
            assert payload["total_amount_settled"] == gross
    finally:
